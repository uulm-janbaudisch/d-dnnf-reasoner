  ...
  crate =
    {
      ...
      pname = lib.concatStringsSep "-" (
        [ "ddnnife" ] ++ lib.optionals d4 [ "d4" ] ++ lib.optionals library [ "uniffi" ]
      );
...
craneLib.${craneAction} (
  ...
  // lib.optionalAttrs (component == "ddnnife_bin" && !hostPkgs.stdenv.hostPlatform.isWindows) {
    postInstall = "mv $out/bin/ddnnife_bin $out/bin/ddnnife";
  }
  // lib.optionalAttrs (component == "ddnnife_bin" && hostPkgs.stdenv.hostPlatform.isWindows) {
    postInstall = "mv $out/bin/ddnnife_bin.exe $out/bin/ddnnife.exe";
  }
